...
st.set_page_config(layout="wide", page_title="DEPI Grid", initial_sidebar_state="expanded")
...
st.markdown("""
    <div class="site-header">
        <div>
            <p class="site-title">DEPI <span>Grid</span></p>
    ...
    if response.status_code == 200:
        ...
        if grid_data:
            ...
            if not wind_df.empty:
                wind_df["ts"] = pd.to_datetime(wind_df["ts"], format="ISO8601")
            if not solar_df.empty:
                solar_df["ts"] = pd.to_datetime(solar_df["ts"], format="ISO8601")
